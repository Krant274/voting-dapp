        ...
        {isAdmin && account && (
          <div className="bg-slate-900 text-white p-7 rounded-[2.5rem] mb-10 shadow-2xl flex flex-wrap gap-6 items-center justify-between border-b-8 border-blue-600">
            ...
            <div className="flex gap-3">
              {electionId > 0 && (
                <button onClick={() => {
                  const n = prompt("Tên ứng viên:"); const d = prompt("Mô tả/Thông điệp:"); if(n) handleAction('registerCandidate', n, d);
                }} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-2xl font-bold text-xs flex items-center gap-2 transition shadow-lg active:scale-95"><PlusCircle size={18} /> BỔ SUNG ỨNG VIÊN</button>
              )}
              
              {votingActive ? (
                <button onClick={() => handleAction('stopVoting')} className="bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-2xl font-bold text-xs flex items-center gap-2 transition shadow-lg active:scale-95"><Power size={18} /> KẾT THÚC BÌNH CHỌN</button>
              ) : (
                <button onClick={() => { 
                  const msg = electionId === 0 ? "Khởi tạo cuộc bầu cử đầu tiên?" : "Thiết lập bình chọn mới? Toàn bộ dữ liệu cũ sẽ được lưu trữ và làm mới hoàn toàn.";
                  if(window.confirm(msg)) handleAction('startNewElection'); 
                }} className="bg-green-600 hover:bg-green-500 text-white px-6 py-3 rounded-2xl font-bold text-xs flex items-center gap-2 transition shadow-lg active:scale-95"><RefreshCw size={18} /> {electionId === 0 ? "KHỞI TẠO HỆ THỐNG" : "THIẾT LẬP BÌNH CHỌN MỚI"}</button>
              )}
            </div>
          </div>
        )}
              ...
              <div className="bg-gradient-to-br from-amber-400 to-orange-500 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden border-4 border-white">
                ...
                <div className="flex flex-wrap justify-center gap-4 mb-4">
                  {winner.names.map((name, idx) => (
                    <h2 key={idx} className="text-5Dashboardxl font-black drop-shadow-2xl italic">{name}{idx < winner.names.length - 1 ? ' & ' : ''}</h2>
                  ))}
                </div>
                <div className="inline-flex items-center gap-2 bg-black/20 px-8 py-3 rounded-full font-black text-2xl border border-white/20">
                  <Trophy size={24} className="text-amber-200" /> {winner.votes} PHIẾU BẦU ĐỒNG THUẬN
                </div>
              </div>
            )}
          </div>
